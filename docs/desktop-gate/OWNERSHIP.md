...
`vela-wt-guards` is gone; its branch survives. `vela-audit-ro` was a detached read-only worktree at
`8200985` for the backlog audit and has been removed now that the audit has reported.
...
| `tests/harness/desktop-click/keys.mjs` (new), `tests/harness/desktop-click/keys.test.mjs` (new), `tests/harness/desktop-click/vela-drive.mjs`, `tests/harness/desktop-click/README.md` | Wave 2 / harness keycode | 2026-08-16 | `keySpecFor` set `keyCode = charCodeAt(0)`, so 32 of the 95 printable ASCII characters were sent with the wrong Windows virtual-key code, and 59 of 95 with at least one wrong field (virtual-key code, `code`, or shift state): the whole run `!`–`/` (33–47) landed on the VK navigation/editing block, `.` was `VK_DELETE`, Chromium ran `DeleteForward` and ate the character. The harness could not type an IP address, so no `reaches-user` evidence about endpoint configuration was ever trustworthy. Fixed with a written-out US-layout table, plus `type --insert-text` as an explicit second route. Worktree `C:/Users/User/vela-w2-harness`, branch `fix/harness-keycode`, based at `8200985`. **Same worktree and directory as the `Wave 2 / click harness` row above — that track's session is finished; this is a follow-on to its output, not a concurrent editor of it.** No Rust, no `package.json`, nothing outside `tests/harness/desktop-click/` except this row |
| `src/runtime/reachable.test.ts` (sole file) | Wave 2 / reachable scope | 2026-08-16 | the guard walked `src/runtime` plus **one named file** in `src/data`, so the rest of the renderer was ungoverned; and it counted `import type` as reachability, which is the defect wearing its own guard. Walk is now the whole of `src/`, value-imports only, with two named-and-justified exemption maps asserted in **both** directions. Worktree `C:/Users/User/vela-w2-skills`, branch `fix/reachable-scope`. **Corrects a stale count that had propagated into the row above and into the guard's own comment: `src/data` holds ONE orphan, not two** — `skills-repository.ts` was wired by Wave 2 / skills surface and is value-reachable from `main.tsx` across 4 edges (5 nodes); only `mcp-repository.ts` remains, and it is named in `AWAITING_A_SURFACE` rather than wired, because there is no `src/features/mcp/` to mount. **Superseded d393a96, which shipped a prose-manufactured-edge defect: the extractor's `[\s\S]*?` head scraped `from '…'` out of doc comments, so a comment on an on-graph file could hide a real orphan — 9 overcounts / 0 undercounts against the TypeScript AST over 265 files. Head class now `[^;/]`** |
| `src-tauri/crates/vela-providers/src/{model,router,event,stream,lib}.rs`, `src-tauri/crates/vela-providers/src/{anthropic,google}/stream.rs`, `src-tauri/crates/vela-store/src/{model,sqlite,migrations}.rs`, `src-tauri/crates/vela-store/src/migrations/0006_answering_endpoint.sql` (new), `src-tauri/crates/vela-store/tests/durability.rs`, `src-tauri/src/ipc/transcript.rs`, `src-tauri/tests/composition_root.rs`, `src/platform/{contract.ts,chat-contract-parity.test.ts,browser-adapter.ts}`, `src/runtime/run-doubles.ts`, `src/features/conversation/{notices.ts,notices.test.ts,turn-stream.ts,turn-stream.test.ts,use-conversation.ts,use-conversation-record.test.tsx,TurnNotices.tsx,MessageTurn.tsx,MessageTurn.test.tsx,ConversationView.tsx,ConversationSurface.tsx,stored-entries.ts,stored-entries.test.ts,EmptyConversation.tsx,EmptyConversation.test.tsx}` (the last one new) | Wave 2 / answer provenance | 2026-08-16 | worktree `C:/Users/User/vela-w2-endpoint`, branch `fix/answer-provenance`, based at `1180710`. A turn addressed to one endpoint could be answered by another and every surface kept naming the one the user chose: `Degradation::FailedOver` carried an attempt count and no identity, and `use-conversation.ts` wrote its own `providerId` *option* into the transcript row. `ChatResponse` gains `answered_by: Option<AnswerProvenance>`, the router stamps it on every success, migration 6 adds `messages.answered_by_{provider,model}_id` **without backfilling** the existing selection into them, and the transcript discloses a substitution. Two collisions, both disclosed: `src/features/conversation/ConversationSurface.test.tsx` is claimed by **Wave 1 / load flake** and was edited by exactly one line — `ChatResponseBody.answeredBy` is required, so its `done()` fixture must state `answeredBy: null`; nothing about its timing or queries was touched. And `src/platform/no-provider-leak.test.ts` is claimed by **desktop** and was **NOT** edited — its §0.3 scan caught the provider-id comparison, so the comparison was moved into `use-conversation.ts`, the one file that scan already exempts, rather than widening the allowlist. `wave2/project-instructions` and `wave2/provider-selection` both hold claims on these conversation and router files; both are ancestors of `1180710` (checked with `git merge-base --is-ancestor`), so this builds on their merged result rather than racing them. **Follow-up on critic review:** `stored-entries.ts` now reads the answering columns back (they were written and read by nothing, so reopening a conversation erased the disclosure), and `EmptyConversation.tsx`'s lede no longer promises *"nothing leaves your machine"* — a claim failover falsifies, on the screen a privacy-motivated user reads before deciding to trust the app |

| `src/features/canvas/CanvasPanel.module.css` (the two `.diffRow[data-kind]` rules only), `src/styles/contrast.test.ts` | Wave 2 / diff contrast | 2026-08-16 | the artifact diff's added and removed rows painted a page-palette tint (`--vela-accent-quiet`, `--vela-danger-bg`) under the code palette's `--vela-code-text`, which is one fixed value in both themes: 1.19:1 and 1.14:1 in light against 4.5:1, 10.06:1 and 13.24:1 in dark. Each row now re-colours with `--vela-text`, the role that turns over with those grounds. **No token value changed and no colour literal was added.** Also closes why it passed: `contrast.test.ts` asserted completeness over the token *vocabulary*, so an audited foreground on an audited background was never measured as a pair — CSS→table, table→CSS and a theme-polarity guard now bracket it. Worktree `C:/Users/User/vela-w2-proj`, branch `fix/diff-contrast`. `src/features/canvas/**` is claimed above by Wave 2 / canvas host boundary, whose work is already merged into the `wave2-complete` tag this branch starts from; that branch touched `.tsx`/`.ts` only and this touches one `.module.css`, so the claims do not overlap in any file |
