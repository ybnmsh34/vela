 ...
 * pool. Measured on this hardware: 5 crashes in 32 consecutive runs, and 1 in 16
 * on a second machine — call it 5-15%, load-dependent. It looks like this, and
 ...
 *
 * Ten of the twelve test files print a result. Two — `capability-matrix.test.ts`
 * and `server.test.ts`, the two slowest — print nothing, and there is no
 * `Tests N passed` summary line at all. The pool tears a forked worker down
 * while the main process still has a message queued for it; the run does not
 * fail, it *stops*.
 *
 ...
 * anything else:
 *
 *   - It retries ONLY when the transcript shows the pool crash AND shows no
 *     failing test. A run with a real failure is returned untouched, first time,
 ...
 *     the attempt count.
 *
 * IF YOU ARE A MAINTAINER LOOKING AT THIS BECAUSE THE STEP WENT RED:
 * the step only fails now if BOTH attempts crashed, or if a test genuinely
 * failed. Search the log for `Tests` — if there is a summary line, the failure
 * is real and is about your change. If there is no summary line and you see
 * `ERR_IPC_CHANNEL_CLOSED` twice, you hit the pool bug twice in a row (~1%) and
 * a re-run is legitimate. It is a vitest/tinypool defect, not repo code; this
 * wrapper should be deleted when it is fixed upstream.
 *
...
/**
 * The pool tore down a worker mid-message. `ERR_IPC_CHANNEL_CLOSED` is the code
 * node raises for a send on a dead IPC channel; tinypool's frame is what makes
 * it this bug rather than something in the harness's own child processes.
 */
 ...
 * crash — a pool that falls over *after* a real failure must not erase it.
 */
